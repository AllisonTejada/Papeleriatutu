...
router.post("/showuserParametros",urlcodeParser,function(req,res){
	...
	req.getConnection((err,conn)=>{
		if(err) return res.send(err)
		const x=""	
		const consulta=x.concat('select * from cliente where cliente_documento="',cliente_documento,'" ')	
...
router.post("/delete",urlcodeParser, function (req, res) {
	var cliente_id_ = req.body.cliente_id;
   
	  console.log(cliente_id_);
...
// // Mostrar Registras tabla agendas - sentencia funcional en postman
router.post("/Mostrar_agendas",urlcodeParser,function(req,res){
	req.getConnection((err,conn)=>{
		if(err) return res.send(err)
